...
export function renderChart(container, { measurement, sex, rangeKey, series, age }) {
  ...
  if (hidden > 0) {
    ...
    note.textContent = hidden === 1
      ? '1 measurement falls outside this age range. Choose a wider range to see it.'
      : `${hidden} measurements fall outside this age range. Choose a wider range to see them.`;
