...
export function renderChart(container, { measurement, sex, rangeKey, points, age }) {
  ...
  for (const p of visiblePoints) {
    ...
    const cls = isSecondary ? 'point point-open' : 'point';

    // Guides down to each axis, the way a value is read off a paper chart.
    // They locate the point among nine curves without decorating it.
    pointGroup.append(el('line', {
      x1: pad.left, y1: cy, x2: cx, y2: cy, class: 'point-guide',
    }));
    pointGroup.append(el('line', {
      x1: cx, y1: cy, x2: cx, y2: pad.top + plotH, class: 'point-guide',
    }));
    ...
    pointGroup.append(el('circle', { cx, cy, r: isSecondary ? 5 : 4.5, class: cls }));
