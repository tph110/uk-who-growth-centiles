...
/**
 * Ages are compared at 4dp throughout. Tabulated ages are stored rounded, so a
 * strict comparison can put an age a fraction of a nanosecond outside its own
 * reference — the 4dp match is what makes day 730 resolve to the last WHO
 * infant row rather than falling into a gap.
 */
...
export function calculate({ age, measurement, sex, observation }) {
  ...
  if (round(age, 4) > round(rows[rows.length - 1][0], 4)) {
    return { error: 'No UK-WHO reference data for this measurement at this age.' };
  }
